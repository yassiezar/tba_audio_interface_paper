...
\Cref{fig:pitch-thresholds} shows the threshold distribution, along with the median value,
%The median is used for each grouping given the skewed plot and
which was found to be approximately 0.4 semitones for each group. \tofix{And \Cref{fig:pitch-thresholds-hist}?}
...
The data is binned in intervals of the effective target width~($w_e$), given by \Cref{eq:fitts-we}, and are plotted for each pitch gradient setting. \tofix{The word 'low' is missing in the figure} 
...
Regarding Group~\textit{G2}¸ we observe larger spreads for each binned data plots, indicating less consistency in the time-to-target results for participants with severe sight impairments.
This could be due to each participant's result taken as a single datum and to the smaller population in Group~\textit{G2}.
...
However, this trend is almost reversed in the time-to-target Fitts's model, where the \textit{lo} setting gives the highest level of performance, followed by the \textit{hi} and \textit{med} settings, respectively. 
...
However, the Fitts's model does not account for different stimuli. We therefore hypothesise that the reason for this divergence in performance is due to the increased resolution of the \textit{hi} setting, which allows for finer adjustments of the device's orientation, getting it closer to the correct target, but at the cost of a higher average time-to-target. 
This seems to indicate a speed/accuracy trade-off in finding the targets. With the Fitts's model discussed here, we can modify the interface to prioritise different metrics and produce the correct output. \tofix{I reworded a few bits, check if still correct}

Regarding the target acquisition, the progressive improvement from the \textit{lo}, \textit{med} and \textit{hi} settings (see \Cref{tab:target-results}) seems to indicate that simply increasing the pitch gradient leads to better target-pointing performance.
However, \Cref{fig:pitch-thresholds-hist} shows that the frequency difference between the ``on-target'' tone and the selected one with the \textit{hi} setting approaches the cut-off frequency of Group~\textit{G1}, indicating the point at which increasing a gradient increase reduces the final performance. 
Indeed, the participants from Group~\textit{G2} seems to go beyond this threshold and reach a saturation point where they can no longer distinguish different tones.  \tofix{I reworded a few bits, check if still correct}
