...
\articleabstract{
  ...
  It has been implemented and tested with spatialized audio cues, which convey the direction of a predefined target in 3D space.
  We present an in-depth evaluation of the audio interface with several experiments that involve a large number of participants, both blindfolded and with actual visual impairments, and analyze the pros and cons of our design choices.
...
A sound source can be spatialized by adjusting a tone's spectral make-up (elevation angle), time delay and level difference (pan angle), and intensity (distance).
...
In previous work, we showed that this audio signal transmission scheme can direct users to a target position with a degree of accuracy comparable to fully spatialized signals used with expensive closed-cup headphones and we expand upon this initial investigation with a larger dataset and look at if, and how, changing the behavior of the pitch affects target acquisition performance in terms of time and angular error.% ~\citep{lock2019bone,macdonald2006spatial}
...
Following this, researchers have investigated using audio signals that are spatialized with a head-related transfer function (HRTF), simulating a sound source located at some arbitrary 3D position~\citep{geronazzo2016interactive,wilson2007swan,katz2010navig,blum2013spatialized}.
The authors generally report favorable results for this guidance approach when used with standard over-ear headphones or speakers. 
...
Cheaper headphones and bone-conduction headphones generally report unfavorable results when compared to over-ear or more expensive alternatives~\citep{schonstein2008comparison,macdonald2006spatial,stanley2006lateralization}. 
...
~\cite{durette2008visuo} transmit a target's elevation angle by adjusting the signal's pitch and report favorable results. 
...
We expand upon this preliminary study with a more thorough investigation into the participants' performance and how changes to the audio signal's behavior can impact that performance.
...
Humans localize a sound source in three dimensions by considering cues recorded in one ear (monaural cues) and comparing cues received at both ears (binaural cues)~\citep{blauert1997spatial,blauert1969sound}.
...
When the modified audio signal enters the inner ear canal, the human brain is able to analyze the frequency response and accurately determine the position of the sound source on the median plane. 
...
Spatialized audio signals are well-suited to the task, displaying similar levels of performance to vocal feedback, but with less cognitive load and higher resolution~\citep{klatzky2006cognitive}.
However, given the previously discussed limitations of bone-conduction and spatialized audio, we propose a simple linear adjustment to the signal's pitch as a function of the elevation angle. 
...
The human audition system uses binaural comparison cues, such as ITD and ILD, to localize a sound source on the horizontal plane~\citep{blauert1969sound}.
...
To transform and spatialize the audio signal, we used OpenAL's default HRTF, based on the MIT's KEMAR dataset~\citep{hiebert2005openal}, which uses the person and targets' positions as input, and outputs a transformed audio signal.
...
The participants were given time before the experiments commenced to familiarize themselves with the device and the tones it emits, as well as what the `on-target' tone sounds like.
...
The results from the experiments we performed allow us to better understand how the users respond to different settings for the spatial audio feedback stimulus and use them to improve and optimize the behavior of the feedback modes in our portable navigation aid.
...
A preliminary set of experiments were conducted to characterize the participants' hearing characteristics.
...
To do this, we played a continuous \SI{512}{\hertz} sinusoidal tone to the participant through the headphones and applied an HRTF to spatialize and place its source to the participant's left or right.
The participant then had to select the direction the sound came from.
The longer the experiment lasts and the more correct guesses the participant makes, the closer the source moves to the centre-front of the participant, making it increasingly harder to localize. 
...
As part of this experiment, we wanted to see how changing the gradient of the pitch function, visualized in~\Cref{fig:pitch-gain}, affects target acquisition performance, e.g.\ does a steeper pitch gain as a function of the elevation angle improve accuracy or decrease the search time?
...
Here we briefly summarize the equations and quantities involved in our metric.
...
\noindent
Here $\theta$ is the angular distance between subsequent target centers and $w_e$ is the targets' effective angular width~\citep{welford1968fundamentals}, given by
...
Both groups were given some time before the target-search experiment to familiarize themselves with the system, the audio signal's behavior and the \SI{512}{\hertz} on-level tone. 
Furthermore, to minimize any potential speed/accuracy biases, we asked the participants to focus on finding the targets without worrying about the time it took to complete the task. 
...
For Group~\textit{G1}, most of the errors were made at the minimum distance from the centre, i.e.\ the most difficult to guess correctly, which is the expected behavior.
...
The results are summarized in \Cref{tab:target-results}.
...
Comparing the distributions for each setting between the two groups with the Kruskal-Wallis test for non-parametric data, we see that the differences between the distributions for all three settings are not significantly different, for both groups and both pan and elevation (the $p$-values are summarized in \Cref{tab:inter-group-results}).
...
The results are summarized in \Cref{tab:fitts-results}.
...
With the Fitts model discussed here, we can modify the interface to prioritize different metrics and produce the correct output. 
...
In this paper we investigated the use of spatialized audio interface with varying pitch to guide a user with visual impairments in a target pointing task.
...
We further tested different pitch settings and found that the user performance in the pan dimension, based on spatialized cues, is independent of such settings.
...
These include questions such as what caused the observed difference in time performance between the groups and whether the constant negative bias observed in \Cref{fig:target-boxplot-error} is indeed caused by a cognitive bias, or whether there is a more complex underlying reason for the behavior.
