...
Both of these devices are shown in \cref{fig:tango-headphone}.
This Tango device uses a set of embedded RGB-D cameras that provide access to powerful real-time localisation, through IMU measurements and landmark tracking, and image-processing capabilities.
...
~
  ...
  \caption{A Tango tablet and the bone-conduction headphones, the primary hardware components used in this work (left), and a participant with visual impairments using the system during an experiment (right)~\citep{lock2019bone}.}
...
This system's guidance instructions can be interpreted in real-time without posing a significant cognitive load to the user, given humans' natural ability to determine a sound source's 3D position.
...
Over the years, commercial and academic parties have devised new and innovative mobile navigation and travel aids for people with visual impairments.
...
However, from an ergonomic perspective, haptic feedback is a weak performer and require significant additional external hardware to transmit guidance instructions with sufficient resolution.
...
Indeed, participants report that they prefer control over the vocal feedback channel and trigger guidance instructions, instead of being given constant guidance instructions~\citep{arditi2013user}.
...
Researchers started using cameras and object detectors to determine what a user is looking at and then use simple audio tones and vocal feedback signals to indicate to the user where to find an object.
...
We found that participants are able to adequately determine a target's elevation with this approach with performance in both pan and elevation dimensions comparable to those of more expensive over-ear headphones.
...
%~\citet{durette2008visuo} transmit a target's elevation angle by adjusting the signal's pitch and report favourable results. 
...
%\todo{include subjective feedback?}
