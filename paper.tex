...
\begin{abstract}
  Sound perception is a fundamentally important skill for many people with severe sight impairments. The research presented in this paper is part of the ActiVis project, the aim of which is to build a mobile guidance aid to help people with limited vision find objects in an unknown environment.
  This system requires an effective non-visual interface and therefore uses bone-conduction headphones to transmit audio instructions to the user.
  ...
  We present an in-depth evaluation of the audio interface with several experiments that involve a large number of participants, both blindfolded and with actual visual impairments. We analyse pros and cons of our design choices, and found that a Fitts's Law metric can be used to improve and refine the quality of the audio interface in future mobile navigation aids. \tofix{Reworded, check if OK}
\end{abstract}
...
~
  ...
  \caption{Pictures of the latest Tango mobile phone (left), the former Tango tablet and the bone-conduction headphones used in this work (centre), and a participant with visual impairments during an experiment (right). \tofix{Update this figures to differ from AEIT)}}
\end{figure}
...
\hfil
  ...
  \caption{The reference system used by the guidance interface showing the camera vector and pan and elevation angles (left) and the pitch gain function used to convey the target's elevation angle (right). Note the logarithmic scale of the frequency axis. \tofix{Update this figure (e.g. use colors) to differ from AEIT)}}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{A diagram of the individual system components and their communication pipelines. $F$ indicates a feedback signal and $P$ a pose signal. \tofix{Update this figure (e.g. use colors) to differ from AEIT)}}\label{fig:pipeline}
\end{figure}
...
We also found that both groups were able to find a set of randomly distribute set of direction targets with similar levels of accuracy.
...
Moreover, we noticed a speed/accuracy trade-off between the settings, where a higher pitch setting produces a smaller angular error, but at the cost of reducing the time performance (i.e. higher time-to-target). 
These results, together with a a Fitts's Law analysis that confirms its applicability to this type of audio interface, provide a useful baseline to improve and refine the latter in future applications, prioritising speed or accuracy to produce the desired output.
\tofix{I reworded a few bits, check if still correct}
...
\tofix{A little more effort in your (hypothetical) plans for future work. You could expand more the user-adaptation concept -- e.g. adapt to different users, or to the performance variations of the same user over time (like the ``progressive co-adaptation'' of~\cite{gallina2015progressive}). Also, what about adding extra modalities, like voice instructions or device vibrations?}
