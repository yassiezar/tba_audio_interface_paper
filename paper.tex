...
\begin{highlighted}The ActiVis project is an effort to create a fully mobile phone-based navigation aid for people with visual impairments to guide them towards a desired object or location within an unknown indoor environment.
...
The proposed system was introduced by~\citet{lock2017portable}, who describe the aforementioned autonomous guidance system paired with a co-adaptive human-machine interface that changes its own parameters over time to better match the advantages and limitations of its main user.
...
However, in this work we examine the effectiveness of the proposed interface for a searching task and investigate a metric that can be used in the next phases of the project to enable the aforementioned co-adaptive paradigm that will possibly benefit the user experience and boost long-term navigation performance.
\end{highlighted}
...
People with visual impairments rely heavily on their hearing~\citep{golledge2004stated} and we wish to avoid blocking this information pathway, since doing to can have severe undesirable effects~\citep{lichtenstein2012headphone}.
...
\begin{highlighted}
Humans are able to localise sounds in three dimensions by extracting different cues from the audio signal~\citep{blauert1969sound,blauert1997spatial}.
...
The interface presents the user with guidance instructions in the form of angular adjustments that are required in the pan and elevation dimensions to point a camera at the desired target (see \cref{fig:cam-coords}).
...
The pitch is changed as a linear function of the elevation angle (see \cref{fig:pitch-gain}), the gradient of which is determined by setting the angle and pitch limits.
...
\todo{mention limitations of Fitts, i.e. zero-width and not playing second tone to confirm target found}

%that we can use as a metric to compare the results between the three configurations. 
...
\noindent \hlbreakable{where $\sigma$ is the standard deviation of the combined pan and elevation error ($x, y$), calculated with}
...
\hlbreakable{The virtual targets have a programmed radius of approximately \SI{10}{\centi\metre} (equivalent to approximately \SI{0.1}{\radian}) and when the participant pointed the camera within this radius, the on-target tone was emitted.
This is similar to the work by~\citet{kabbash1995prince}, where they had participants search for a point-target with an oversized cursor and found that this was similar to a Fitts-like response.
...
\hlbreakable{Group~\textit{G1} consisted of 42 undergraduate students (10 male, 32, female) with normal eyesight who were blindfolded for the experiments (mean age: $20\pm2$).
...
\hlbreakable{The Pearson correlation scores for Group~\textit{G1} indicate a moderate to strong positive correlation between the target and the selected locations ($r_{pan} \in [0.72, 0.77],~p < 0.001;~r_{elevation} \in [0.36, 0.49],~p < 0.001$), showing that both the pan and elevation cues in general worked as expected.}
...
\hlbreakable{Furthermore, the significant variance in the error results for Group \textit{G2} could possibly be because of the higher mean age compared to Group \textit{G1} and their general inexperience with mobile phones.
...
The similarity between the differences is further highlighted with non-significant results from the two-sided Kolmogorov-Smirnov test ($p > 0.05$ for both groups and all three test conditions).\todo{include these p-values in the table?}
\hlbreakable{This result confirms that the performance of the blindfolded participants and those with severe sight impairments are statistically similar despite the seemingly clear differences in error performance.
...
\begin{table}
  \centering
  \caption{A summary of the $p$-values from the Kruskal-Wallis test comparing the distributions of the different settings' error data for each group in both the pan and elevation dimensions. }\label{tab:inter-group-results}
  \begin{tabular}{lll}
    \toprule
...
\hlbreakable{This is confirmed by strong Pearson correlation scores for each setting ($r_{lo} = 0.98,~p_{lo}<0.001;~r_{med}=0.94,~p_{med}<0.001;~r_{hi} = 0.97,~p_{hi}<0.001$).
...
\hlbreakable{This is supported by the results from the Friedman test, showing that there is a significant difference in performance between the settings ($p < 0.001$), as well as post-hoc Wilcoxon tests with Holm-Bonferroni corrections, which show that the \textit{lo} setting is significantly different to the \textit{med} and \textit{hi} settings ($p_{lo-med} < 0.001,~p_{lo-hi} < 0.001$). 
The \textit{med} and \textit{hi}, instead, are not significantly different from each other ($p_{med-hi} = 0.85$).}
