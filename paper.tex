...
In~\cite{lock2019bone}, we showed that this audio signal transmission scheme can direct users to a target position with a degree of accuracy comparable to fully spatialised signals sued with expensive closed-cup headphones~\cite{macdonald2006spatial}.
...
We then used each CDF's parameters to determine a frequency cut-off threshold where the participant could not longer reliably tell tones apart and is set to contain 75\% of each participant's correct guesses.% and an example of one participant's threshold can be seen in \todo{put in figure}.
...
\begin{table}
  ...
  \begin{tabular}{p{0.5cm}p{0.7cm}p{0.7cm}p{1cm}p{2cm}p{2.25cm}}
    \toprule
    ...
    &           & \textit{hi}  & $-0.03\pm0.39$ & $0.26\pm0.29$ & $0.72, p < 0.001$ \\ \cline{3-5}
    ...
    &           & \textit{hi}  & $ 0.03\pm0.48$ & $0.36\pm0.22$ & $0.21, p < 0.001$ \\\cline{3-5}
...
A similar pattern is observed for Group \textit{G2}, but the improvement across the settings is more subtle and not as linear as for Group \textit{G1}.
\cref{fig:target-boxplot-absolute-errors} shows a clearer improvement, i.e.\ approaching \SI{0}{\radian}, for the elevation data between the 3 settings in both groups.
...
\todo{discuss post-hoc in more detail}
...
\begin{table}
  ...
  \begin{tabular}{lll}
    \toprule
      ...
      \textit{lo}  & $p=0.77$ & $p=0.95$ \\
      \textit{med} & $p=0.98$ & $p=0.07$ \\
      \textit{hi}  & $p=0.72$ & $p=0.41$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
To investigate if the interface generates a Fitts-like responce from the participants, we plot the time to find the target as a function of the targets' indices of difficulty as defined by \cref{eq:fitts-base}.
...
This is confirmed with strong Pearson correlation scores for each setting ($r_{lo} = 0.0.76, p_{lo} = 0.045; r_{med} = 0.96, p_{med}<0.001; r_{hi} = 0.86, p_{hi} = 0.013$).
...
For the results from Group\textit{G1}, there is a progressive improvement in performance between the 3 settings, with the \textit{lo} producing the highest indices of performance overall, i.e.\ the participants found the targets with the smallest error in the least about of time with the \textit{lo} setting.
...
The results for Group \textit{G2} shows generally lower indices of performance for each setting, which is expected given the increased times to target observed in \cref{fig:fitts-results}.
Again, the \textit{lo} setting produces the highest performance by a large margin compared to the \textit{med} setting with the Wilcoxon test with Holm-Bonferronni corrections ($p_{lo-med}=0.01$). 
However, the same test indicates that there is not a statistical difference between the \textit{lo} and \textit{hi} or the \textit{med} and \textit{hi} settings ($p_{lo-hi} = 0.1, p_{med-hi} = 0.038$).
...
\todo{compare inter-group results}
